...
async function translateText(text, accessKey, secretKey) {
  ...
  try {
    ...
    console.log('请求方法:', signParams.method);
    ...
    if (data.ResponseMetadata?.Error) {
      throw new Error(`翻译API错误: ${data.ResponseMetadata.Error.Message} (Code: ${data.ResponseMetadata.Error.Code})`);
    }
...
async function testTranslate() {
  ...
  console.log('开始测试火山翻译API...\n');

  for (let i = 0; i < testTexts.length; i++) {
    const text = testTexts[i];
    console.log(`\n=== 测试 ${i + 1}/${testTexts.length} ===`);
    console.log(`原文: "${text}"`);
    
    try {
      const translation = await translateText(text, accessKey, secretKey);
