...
// 增强的Vue转换实现
const transformVueEnhanced = async (data) => {
  ...
  try {
    // 使用更精确的解析方法
    const parseResult = parseVueFileEnhanced(sourceCode);
    ...
    if (parseResult.template) {
      const templateResult = processTemplateBlockEnhanced(parseResult.template, keyPrefix, i18nMap, quoteKeys);
    ...
    if (parseResult.script) {
      const scriptResult = processScriptBlockEnhanced(parseResult.script, keyPrefix, i18nMap, quoteKeys);
...
// 增强的Vue文件解析
const parseVueFileEnhanced = (sourceCode) => {
  const result = {
    template: null,
    script: null,
    scriptSetup: null,
...
// 增强的template处理
const processTemplateBlockEnhanced = (template, keyPrefix, i18nMap, quoteKeys) => {
  try {
...
// 增强的script处理 - 使用更精确的解析
const processScriptBlockEnhanced = (script, keyPrefix, i18nMap, quoteKeys) => {
  try {
...
const testEnhancedVersion = async () => {
  ...
  try {
    ...
    // 执行转换
    const result = await transformVueEnhanced({ path: complexFilePath });
...
// 运行测试
testEnhancedVersion().catch(console.error); 